    ...
    public class ChatBox : UIElement
    {
        ...
        public override void DrawGL(ViewMatrixes vm, params TextureUnit[] tu)
        {
            if (!_loaded || _disposed)
            {
                if (!_warnOnce)
                {
                    Logger.WriteToLog("Attempt to draw unloaded ChatBox object: {0}", this);
                    Console.WriteLine("Attempt to draw unloaded ChatBox object: {0}", this);
                    DebugConsole.WriteLine("Attempt to draw unloaded ChatBox object: {0}", this);
        ...
        protected override void DrawBorderGL(ViewMatrixes vm)
        {
            ...
            {
                Console.WriteLine("Cannot draw border for {0} when default color shader is null.", nameof(ChatBox));
                Logger.WriteToLog("Cannot draw border for {0} when default color shader is null.", nameof(ChatBox));
                DebugConsole.WriteLine("Cannot draw border for {0} when default color shader is null.", nameof(ChatBox));
                return;
            }
